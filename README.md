...
This is a grid search in eight directions. Orthogonal and diagonal steps each take one tick. The dose is the sum of the costs in the caller's table. The line prints that sum, the number of edges that were added, then the nodes. Costs are not negative, so the first time the goal leaves the heap is the minimum sum. `stay` means no walk was accepted, so there is no dose. A missing edge is not a step. A negative cost is not a step. A non-finite cost is not a step. A non-finite dose cap is a stay, not an open cap. The route is the cheapest walk that finishes inside the time. If every walk breaks the clock, or the cheapest walk breaks the dose cap, the desk says stay.
